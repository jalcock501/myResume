...
git config --global user.email "jalcock501@googlemail.com"
git config --global user.name "Jim Alcock"

# Web app file structure
mkdir /apps
chmod 777 /apps
mkdir /apps/logs
mkdir /apps/logs/myResume
mkdir /apps/logs/myResume/app_log
cd /myResume
...
# clone the repo:
cd /myResume
git clone https://github.com/jalcock501/myResume resume_repo
...
cp /apps/resume_repo/server/myResume.service /etc/systemd/system/myResume.service

systemctl start myResume
systemctl status myResume
systemctl enable myResume
...
cp /apps/resume_repo/server/myResume.nginx /etc/nginx/sites-enabled/myResume.nginx
